...
router.post('/data', async (req, res) => {
    try {
        ...
        // Check if this is a master or slave drone
        const masterInfo = await masterSlaveService.getMasterDrone();
        const isMaster = masterInfo && masterInfo.masterUAVId === uavData.uavId;
        
        console.log(`📡 Received UAV data from ${uavData.uavId} ${isMaster ? '(MASTER)' : '(SLAVE)'} - ISAC: ${uavData.isacMode?.toUpperCase()} (${uavData.signalStrength?.toFixed(1)}%)`);
        
        // If this is a slave drone, route data to master
        if (!isMaster && masterInfo) {
            console.log(`📤 Routing slave ${uavData.uavId} data to master ${masterInfo.masterUAVId}`);
            
            // Store slave data
            await masterSlaveService.storeSlaveData({
                slaveUAVId: uavData.uavId,
                masterUAVId: masterInfo.masterUAVId,
                ...
                detections: uavData.detections || [],
                timestamp: uavData.timestamp
            });
            
            // Emit slave data received event
        ...
        // If master, get aggregated data from all slaves and send complete data
        if (isMaster) {
            const aggregatedData = await masterSlaveService.getAggregatedData(uavData.uavId, 100);
            
            // Emit master aggregated data
            req.io.emit('master_aggregated_data', {
                masterUAVId: uavData.uavId,
                masterData: {
                    location: uavData.location,
                    batteryLevel: uavData.uavStatus?.batteryLevel,
                    isacMode: uavData.isacMode,
            ...
            console.log(`👑 Master ${uavData.uavId} aggregated data: ${aggregatedData.totalSlaves} slaves, ${aggregatedData.totalDetections} detections`);
        }
        ...
        res.json({
            success: true,
            message: isMaster ? 'Master UAV data processed successfully' : 'Slave UAV data routed to master',
            role: isMaster ? 'master' : 'slave',
            masterUAVId: isMaster ? uavData.uavId : (masterInfo ? masterInfo.masterUAVId : null),
            processed: {
