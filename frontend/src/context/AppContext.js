...
const initialState = {
  ...
  error: null,
  masterUAVId: 'UAV-001' // Initial master drone
};
...
const appReducer = (state, action) => {
  switch (action.type) {
    ...
    case actionTypes.SET_MASTER_UAV:
      return {
        ...state,
        masterUAVId: action.payload
      };
...
export const AppProvider = ({ children }) => {
  ...
  // Rotate master drone randomly every 15 seconds
  useEffect(() => {
    if (!state.uavs || state.uavs.length === 0) {
      console.log('⚠️ No UAVs available for master rotation');
      return;
    }

    console.log(`🚀 Starting master drone rotation. Current master: ${state.masterUAVId}, Total UAVs: ${state.uavs.length}`);

    const rotateMasterDrone = () => {
      // Use ref to get latest state values
      const currentState = stateRef.current;
      
      if (!currentState.uavs || currentState.uavs.length === 0) {
        console.log('⚠️ No UAVs available in rotation');
        return;
      }
      
      // Get all available UAV IDs except the current master
      const availableUAVIds = currentState.uavs
        .map(uav => uav.uavId)
        .filter(uavId => uavId !== currentState.masterUAVId);
      
      console.log(`🔄 Rotation check - Current master: ${currentState.masterUAVId}`);
      console.log(`📊 Available UAVs for selection: ${availableUAVIds.join(', ')}`);
      
      // If there are other UAVs available, randomly select one
      if (availableUAVIds.length > 0) {
        const randomIndex = Math.floor(Math.random() * availableUAVIds.length);
        const nextMasterUAVId = availableUAVIds[randomIndex];
        
        console.log(`✅ Rotating master drone: ${currentState.masterUAVId} → ${nextMasterUAVId}`);
        
        dispatch({
          type: actionTypes.SET_MASTER_UAV,
          payload: nextMasterUAVId
        });

        // Show notification when master changes
        dispatch({
          type: actionTypes.ADD_NOTIFICATION,
          payload: {
            type: 'info',
            message: `Master drone changed to ${nextMasterUAVId}`,
            duration: 3000,
            id: Date.now()
          }
        });
      } else {
        // Only one UAV available, keep it as master
        console.log(`⚠️ Only one UAV available, keeping ${currentState.masterUAVId} as master`);
      }
    };

    // Rotate every 15 seconds (don't rotate immediately, wait 15 seconds first)
    const masterRotationInterval = setInterval(rotateMasterDrone, 15000);
    
    console.log(`⏰ Master rotation interval set for every 15 seconds`);

    return () => {
      console.log('🛑 Clearing master rotation interval');
      clearInterval(masterRotationInterval);
    };
  }, [state.uavs.length, dispatch]); // Only depend on UAV count
