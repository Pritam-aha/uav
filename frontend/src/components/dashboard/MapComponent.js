...
const createUAVIcon = (isacMode, isMaster = false) => {
  if (!L) return null;
  ...
  // Master drone gets special styling
  const size = isMaster ? 24 : 16;
  const borderWidth = isMaster ? 3 : 2;
  const borderColor = isMaster ? '#fbbf24' : 'white';
  const shadow = isMaster ? '0 4px 8px rgba(251, 191, 36, 0.5)' : '0 2px 4px rgba(0,0,0,0.3)';
  ...
  return L.divIcon({
    ...
    html: `<div style="
      ...
      position: relative;
    ">
      ${isMaster ? '<div style="position: absolute; top: -8px; left: 50%; transform: translateX(-50%) rotate(-45deg); font-size: 12px;">👑</div>' : ''}
    </div>`,
    iconSize: [size, size],
...
const MapComponent = ({ survivors, uavStatus, uavs, onSurvivorClick, masterUAVId }) => {
  ...
  return (
    ...
    >
      ...
      {uavs && uavs.map((uav) => {
        const isMaster = uav.uavId === masterUAVId;
        return (
          uav.location && uav.location.lat && (
            <Marker 
              key={uav.uavId}
              position={[uav.location.lat, uav.location.lng]} 
              icon={createUAVIcon(uav.isacMode, isMaster)}
            >
              <Popup>
                <div>
                  <div className="popup-title">
                    {isMaster && <span style={{ marginRight: '4px' }}>👑</span>}
                    {uav.uavId}
                    {isMaster && <span style={{ 
                      marginLeft: '8px', 
                      fontSize: '0.75rem', 
